...
{
    public class EliminarPartidaCommandHandler:IRequestHandler<EliminarPartidaCommand>
    {
        private readonly IUnitOfWork _unitOfWork;
        private readonly ICfdiRepository _cfdiRepository;
        public EliminarPartidaCommandHandler(IUnitOfWork unitOfWork, ICfdiRepository cfdiRepository)
        {
            _unitOfWork = unitOfWork ?? throw new ArgumentNullException(nameof(unitOfWork));
            _cfdiRepository = cfdiRepository ?? throw new ArgumentNullException(nameof(cfdiRepository));
        }

        public async Task<Unit> Handle(EliminarPartidaCommand request, CancellationToken cancellationToken)
        {
            using (var transactionScope = new TransactionScope(TransactionScopeAsyncFlowOption.Enabled))
            {
                ...
                cfdi.EliminarPartida(request.PartidaId);
